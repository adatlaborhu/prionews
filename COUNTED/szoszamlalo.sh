...
abc_article=$(cat /home/mtomi/IGAZHIR/JOINED/joined_dated_filtered.csv |head -2 |tail -1 |cut -d' ' -f1)
bbc_article=$(cat /home/mtomi/IGAZHIR/JOINED/joined_dated_filtered.csv |head -2 |tail -1 |cut -d' ' -f2)
cnn_article=$(cat /home/mtomi/IGAZHIR/JOINED/joined_dated_filtered.csv |head -2 |tail -1 |cut -d' ' -f3)
#echo $abc_article
#echo $bbc_article
#echo $cnn_article
...
cat joined_percents2.csv |awk '{print $1" "($2 + $3 + $4)/3}' |sort -rnk2 |sort > average_values2.csv
...
#big diff
abc_percent=$(cat abc_diff.csv| cut -d' ' -f2 |tail -n+2 |paste -sd+ | bc |awk '{print 1 - $1}')
bbc_percent=$(cat bbc_diff.csv| cut -d' ' -f2 |tail -n+2 |paste -sd+ | bc |awk '{print 1 - $1}')
cnn_percent=$(cat cnn_diff.csv| cut -d' ' -f2 |tail -n+2 |paste -sd+ | bc |awk '{print 1 - $1}')
echo $abc_percent $abc_article
echo $bbc_percent $bbc_article
echo $cnn_percent $cnn_article